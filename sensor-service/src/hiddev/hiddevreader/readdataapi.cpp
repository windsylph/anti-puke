...
namespace kmicki::hiddev
{
    ...
    void HidDevReader::ReadDataApi::Execute()
    {
        ...
        if(!dev.Open())
        {
            // Throwing here would escape the thread and abort the process. The
            // overwhelmingly likely cause is that we cannot open /dev/hidraw*,
            // so say so rather than leaving the user with a SIGABRT.
            motiondots::FatalError(
                "cannot open the Steam Deck HID device (vid 28de, pid 1205). "
                "Usually this means insufficient permissions to read /dev/hidraw*: "
                "run the service as root, or install a udev rule granting access. "
                "It can also mean the device is already open by another process.");
            return;
        }
